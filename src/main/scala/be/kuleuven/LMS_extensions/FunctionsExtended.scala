...
trait FunctionsExt extends Functions {
  ...
  def doLambdaEntry[A:Typ,B:Typ](name: String)(fun: Rep[A] => Rep[B])(implicit pos: SourceContext): Rep[A => B]
...
trait FunctionsExpExt extends FunctionsExp with FunctionsExt {
  ...
  case class NamedLambdaEntry[A:Typ,B:Typ](name: String, f: Exp[A] => Exp[B], x: Exp[A], y: Block[B]) extends Def[A => B] { val mA = manifest[A]; val mB = manifest[B] }
  ...
  override def doApplyDecl[A:Typ,B:Typ](f: Exp[A => B])(implicit pos: SourceContext): Exp[B] = {
    ...
    f match {
      ...
      case Def(NamedLambdaInput(_,_,_,y)) =>
        val ye = summarizeEffects(y)
        reflectEffect(ApplyDecl(f), ye)
      case Def(NamedLambdaEntry(_,_,_,y)) =>
        val ye = summarizeEffects(y)
        reflectEffect(ApplyDecl(f), ye)
      case _ =>
  ...
  def doNamedLambdaEntryDef[A:Typ,B:Typ](name: String)(f: Exp[A] => Exp[B]) : Def[A => B] = {
    val x = unboxedFresh[A]
    val y = reifyEffects(f(x)) // unfold completely at the definition site.

    NamedLambdaEntry(name, f, x, y)
  }

  override def doLambdaEntry[A:Typ,B:Typ](name: String)(f: Exp[A] => Exp[B])(implicit pos: SourceContext): Exp[A => B] =
    doNamedLambdaEntryDef(name)(f)
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    ...
    case e@NamedLambdaEntry(n, g,x:Exp[Any],y:Block[b]) => toAtom(NamedLambdaEntry(n, f(g),f(x),f(y))(e.mA,e.mB))(mtype(manifest[A]),pos)
  ...
  override def syms(e: Any): List[Sym[Any]] = e match {
    ...
    case NamedLambdaEntry(_, f, x, y) => syms(y)
  ...
  override def boundSyms(e: Any): List[Sym[Any]] = e match {
    ...
    case NamedLambdaEntry(_, f, x, y) => syms(x) ::: effectSyms(y)
  ...
  override def symsFreq(e: Any): List[(Sym[Any], Double)] = e match {
    ...
    case NamedLambdaEntry(_, f, x, y) => freqHot(y)
...
trait TupledFunctionsExt extends TupledFunctions with FunctionsExt {
  ...
  def entryfun0[B:Typ](name: String)(f: () => Rep[B]): Rep[Unit=>B] =
    doLambdaEntry(name)((t: Rep[Unit]) => f())
...
trait TupledFunctionsExpExt extends TupledFunctionsExp with TupledFunctionsExt with FunctionsExpExt {
  override def boundSyms(e: Any): List[Sym[Any]] = e match {
    ...
    case NamedLambdaEntry(_, f, UnboxedTuple(xs), y) => xs.flatMap(syms) ::: effectSyms(y)
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    ...
    case e@NamedLambdaEntry(n, g,UnboxedTuple(xs),y:Block[b]) => toAtom(NamedLambdaEntry(n, f(g),UnboxedTuple(f(xs))(e.mA),f(y))(e.mA,e.mB))(mtype(manifest[A]),implicitly[SourceContext])
...
trait CGenFunctionsExt extends CGenFunctions {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NamedLambdaInput(_, fun, x, y) =>
      ...
      stream.println("};")
    case NamedLambdaEntry(_, fun, x, y) =>
      val retType = remap(getBlockResult(y).tp)
      stream.println(retType + " " + quote(sym) + "(" + remap(x.tp) + " " + quote(x) + ") {")
      emitBlock(y)
      val z = getBlockResult(y)
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NamedLambdaInput(_, fun, UnboxedTuple(xs), y) =>
      ...
      if (retType != "void")
        stream.println("return " + quote(z) + ";")
      stream.println("};")
    case NamedLambdaEntry(_, fun, UnboxedTuple(xs), y) =>
      val retType = remap(getBlockResult(y).tp)
      stream.println(retType + " " + quote(sym) + "(" + xs.map(s=>remap(s.tp)+" "+quote(s)).mkString(",") +") {")
      emitBlock(y)
      val z = getBlockResult(y)
...
trait SMCGenFunctionsExt extends CGenFunctions {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NamedLambdaInput(name, fun, x, y) =>
      ...
      emitBlock(y)
      stream.println("}")
    case NamedLambdaEntry(name, fun, x, y) =>
      val retType = remap(getBlockResult(y).tp)
...
trait SMCGenTupledFunctionsExt extends CGenFunctionsExt with GenericGenUnboxedTupleAccess {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NamedLambdaInput(name, fun, UnboxedTuple(xs), y) =>
      ...
      emitBlock(y)
      stream.println("}")
    case NamedLambdaEntry(name, fun, UnboxedTuple(xs), y) =>
      val retType = remap(getBlockResult(y).tp)
